...
> **See `ZERO_PLASTIC_UPDATED_REQUIREMENTS_V3_WEB_VS_MOBILE.md` → Addendum (July 2026)** for the latest confirmed decisions (Chapter=Club, Events vs Projects, HQ task tracker, no AI automation, Corporate Portal instead of public API, no donations, notification broadcast rules, segmented flash reward drops, Professional/Corporate Volunteer registration profile). This document's content otherwise stands.
>
> **Note on scope:** everything below assumes the "Ground Volunteer" persona (the one who checks in at a cleanup). A second, much thinner persona now exists at registration — the **Professional/Corporate Volunteer** — who registers workplace/industry/expertise instead of a Club, never checks in, and is only reachable via a District-level directory admins search when they need expert help. That persona doesn't change anything below; it's a separate registration fork (Addendum #13), not a new mobile screen for this app's core flows.
...
#### 9. **Catch a Flash Reward Before It's Gone**
```
Scenario: My chapter has 5 t-shirts to give away, first come first served
Solution:
├─ Post shows up in my feed — but only if I'm in the eligible group
│   (e.g. "1000+ marks" volunteers get this one; "<1000 marks" volunteers
│   get a different post nudging them to earn more)
├─ Shows a countdown and "3 of 5 left"
├─ One tap: CLAIM
├─ Instant result: "You got it! 🎉" or "All claimed — you were too late"
└─ 10 seconds total, feels like grabbing a deal on social media

Why this matters: admins/district leaders don't want to spam every
volunteer with every perk — only the volunteers who actually qualify
should see it, and the scarcity (5 shirts, ticking clock) is what makes
someone open the app right now instead of "later."
```
...
VOLUNTEER_APP = {
  ...
  Notifications: {
    ...
    ✅ Tier promotions
    ✅ New projects nearby
    ✅ Flash reward posts (segmented by marks/points, first-come-first-served claim)
  },
...
WEB_APP = {
  ...
  Integrations: {
    ✅ Monday.com board
    ✅ Twilio SMS/WhatsApp (basic messaging only, no automation/AI)
    ✅ Email notifications
  },
...
### MOBILE APP (primary path — also available via web URL per Addendum #9, no app required)

```
✅ Check-in (GPS required — mobile native or browser geolocation on web)
✅ Check-out (GPS required — mobile native or browser geolocation on web)
...
──────────────────────────────────────────────────────────────
Volunteer discovers projects  ✅ Yes    ✅ Yes    MOBILE, WEB URL as fallback
Volunteer accepts project     ✅ Yes    ✅ Yes    MOBILE, WEB URL as fallback
Volunteer checks in           ✅ Yes    ✅ Yes    MOBILE (GPS), WEB (browser geolocation)
Volunteer checks out          ✅ Yes    ✅ Yes    MOBILE (GPS), WEB (browser geolocation)
