...
> **Latest clarifications live in `ZERO_PLASTIC_UPDATED_REQUIREMENTS_V3_WEB_VS_MOBILE.md` → Addendum (July 2026)** and are mirrored in `ZERO_PLASTIC_FINAL_ARCHITECTURE_SUMMARY_V3.1.md`. Key points: Chapter=Club, Events are distinct from but nestable with Projects, HQ gets a Trello-style task tracker, no AI automation, a Corporate Benefits & Offers program with membership-ID redemption, no donations, volunteers get full no-app web parity including web-based check-in/out, every project has a public blog-style page (which the Corporate Portal links out to) with authority-scoped notification broadcasting, admin/district-level users can post segmented, time-bound, limited-quantity "Flash Reward" drops (first-come-first-served) targeted by points/marks criteria, and volunteers can register as a "Professional/Corporate Volunteer" (workplace, industry, expertise tags, submitted at District level) forming a searchable expert pool for partnership/support outreach instead of ground project work.
...
**Decision:** Mobile is the primary path for check-in, badges, certificates — but a plain web URL (no app install) gives volunteers the same actions, including GPS check-in/out via browser geolocation (Addendum #9)
